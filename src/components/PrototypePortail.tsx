...
export default function PrototypePortail() {
  ...
  const login = () => {
    ...
    if(!u){ alert("Email inconnu."); return; }
    setMe(u);
  };
  const logout = () => setMe(null);

  const role:Role|null = me?.role ?? null;
  const isAdmin = role==="admin", isManager = role==="manager", isEmployee = role==="employee";
  ...
  const requestAbs = ()=> {
    if(!me) return; if(!newAbs.from || !newAbs.to) return;
        ...
        <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-6">
          <section className="w-full max-w-lg rounded-3xl border border-neutral-800 bg-[#0d1118] shadow-[0_10px_30px_rgba(0,0,0,.45)] overflow-hidden">
            <header className="px-7 py-7">
              <h1 className="text-3xl font-extrabold tracking-wide text-center">Connexion</h1>
            </header>
            <div className="px-7 pb-7 pt-1 border-t border-neutral-800 bg-[#0f1319] space-y-5">
              ...
              <p className="text-xs text-neutral-500 text-center">Astuce : employee@test.com · manager@test.com · admin@test.com</p>
            </div>
          </section>
        </div>
      ) : (
        <main className="mx-auto max-w-[1600px] px-6 py-10">
          {/* 3 colonnes égales — même hauteur */}
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
            {/* ========== COL 1 : ANNONCES ========== */}
            <Panel title="ANNONCES">
              <div className="flex flex-col h-full">
                {(isAdmin||isManager) && (
                  <div className="p-5 rounded-2xl border border-neutral-700 bg-[#11151d]">
                    <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-4">
                      ...
                      <textarea className="sm:col-span-2 min-h-[110px] px-4 py-3 rounded-lg border border-neutral-700 bg-[#0b0f15]" placeholder="Contenu"
            ...
            <Panel title="ABSENCES">
              <div className="flex flex-col h-full">
                {isEmployee && (
                  <div className="p-5 rounded-2xl border border-neutral-700 bg-[#11151d] space-y-4">
                    ...
                    <textarea className="w-full min-h-[100px] px-4 py-3 rounded-lg border border-neutral-700 bg-[#0b0f15]" placeholder="Motif (optionnel)"
                            ...
                            <tr key={a.id} className="border-t border-neutral-800 bg-[#0b0f15]">
                              ...
                              <td className="p-3">
                                <div className="flex items-center gap-2">
                                  <span className={`text-[12px] px-2 py-0.5 rounded-full border ${cls}`}>{label}</span>
            ...
            <Panel title="DIRECTION">
              <div className="flex h-full gap-5">
                ...
                <nav className="w-72 shrink-0">
                  <Scroller>
                    {partners.map(p=>{
                      ...
                      return (
                        <button key={p.id}
                                className={`w-full text-left p-4 rounded-xl border transition mb-4 ${sel?"bg-[#141923] border-neutral-600":"bg-[#0b0f15] border-neutral-800 hover:border-neutral-600"}`}
                                onClick={()=>setPeerId(p.id)}>
                          <div className="flex items-center gap-3">
                            <span className="h-9 w-9 rounded-full bg-[#141923] border border-neutral-700 flex items-center justify-center text-sm">{p.name.slice(0,2).toUpperCase()}</span>
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  ...
                  {/* Input collé en bas */}
                  <form className="mt-4 flex items-center gap-4" onSubmit={(e)=>{e.preventDefault();send();}}>
...
function TopBar({me,role,logout}:{me:UserT|null; role:Role|null; logout:()=>void}) {
  return (
    <header className="border-b border-neutral-800" style={{background:"#0d1118"}}>
      <div className="mx-auto max-w-[1600px] h-16 px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
...
/* Carte colonne — hauteur rigide identique partout */
function Panel({title,children}:{title:string;children:React.ReactNode}) {
  return (
    <section className="rounded-[28px] border border-neutral-800 bg-[#0d1118] shadow-[0_14px_40px_rgba(0,0,0,.5)] overflow-hidden h-[880px] flex flex-col">
      <header className="px-7 pt-6 pb-4">
        <h2 className="text-[26px] font-extrabold tracking-wide">{title}</h2>
      </header>
      <div className="px-6 pb-6 pt-2 border-t border-neutral-800 bg-[#0f1319] flex-1 flex flex-col min-h-0 overscroll-contain">
        {children}
      </div>
    </section>
  );
...
/* Wrapper scroll interne uniforme */
function Scroller({children}:{children:React.ReactNode}) {
  return (
    <div className="mt-5 flex-1 overflow-auto pr-2 space-y-5 min-h-0">
      {children}
    </div>
  );
}
